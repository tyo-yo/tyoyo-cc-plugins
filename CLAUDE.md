...
**ベストプラクティス**:
- `name` は **必ず kebab-case**（小文字・ハイフン区切り、最大64文字）。スペースや大文字を含むとスラッシュコマンドとして認識されない（例: `knowledge-pathfinder` ✅ / `Knowledge Pathfinder` ❌）
...
プラグインのファイルを修正したら、**必ず**そのプラグインのバージョンを更新する（semver の patch をインクリメント）。

**更新が必要な2箇所:**

1. `<plugin-name>/.claude-plugin/plugin.json` の `"version"`
2. `.claude-plugin/marketplace.json` の該当プラグインエントリの `"version"`

両方を同じバージョンに揃えること。

**バージョニング基準:**
- patch（例: 1.0.0 → 1.0.1）: バグ修正、文言修正、小さな改善
- minor（例: 1.0.0 → 1.1.0）: 新機能追加、新コマンド/スキル/エージェント追加
