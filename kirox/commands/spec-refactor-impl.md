...
2. `.kiro/specs/{feature}/tasks.md` から実装ファイルを抽出（存在する場合）
3. `git log --name-only -10` で直近の変更ファイルとコミットメッセージを確認
4. テストファイルを除外した **実装ファイルリスト** を確定し、ユーザーに明示

## 3パス実行

refactor-impl-agent を3回直列で呼び出す。各パスの出力はそのまま表示する。
...
承認された決定は `/Users/tyoyo/repos/tyoyo-cc-plugins/kirox/skills/kirox/references/refactor-impl-rules.md` に追記する。
